...
// 回溯法，思路应该没什么问题 Time Limit Exceeded
class Solution {
	int recurse(int index, int& end, bool isBuy, bool isCooldown, vector<int>& prices) {
		// 遍历结束
		if (index == end) {
			return 0;
		}
		// 必须先冷静
		if (!isCooldown) {
			return recurse(index + 1, end, isBuy, true, prices);
		}
		// 买过
		if (isBuy) {
			// 卖或者不卖
			return max(prices[index] + recurse(index + 1, end, false, false, prices), recurse(index + 1, end, true, true, prices));
		}
		// 没买过
		else {
			// 买或者不买
			return max(-prices[index] + recurse(index + 1, end, true, true, prices), recurse(index + 1, end, false, true, prices));
		}
	}
public:
	int maxProfit(vector<int>& prices) {
		int end = prices.size();
		return recurse(0, end, false, true, prices);
	}
};
...
// Memory Usage : 8.5 MB, less than 100.00 % of C++ online submissions for Best Time to Buy and Sell Stock with Cooldown.
class Solution2 {
public:
	int maxProfit(vector<int>& prices) {
...
//	cout << "最大收益是: " << Solution2().maxProfit(prices) << endl;
//	return 0;
//}